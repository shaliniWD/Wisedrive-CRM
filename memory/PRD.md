...
## Changelog
- v5.1 (Feb 22, 2026): **Lead Investigator Audit Trail + AD ID Auto-Generation** - Two critical fixes for Meta WhatsApp leads: (1) **Bug Fix** - Fixed NameError in webhook by adding missing To, AccountSid, NumMedia Form parameters. (2) **Audit Trail Storage** - Webhook now stores comprehensive audit data with 6 sections: raw_twilio_params, parsed_standard_fields, parsed_ctwa_fields, extraction_log, city_lookup_log, final_assignment. (3) **AD ID Auto-Generation** - When Twilio doesn't send CtwaClid (common issue), system now auto-generates AD ID from ad_name (ReferralHeadline). Format: `auto_{ad_name}_{date}`. (4) **Lead Investigator UI** - New expandable "Webhook Audit Trail" section shows all debugging data with color-coded status. 100% test coverage (iteration_65, iteration_66). Files: /app/backend/server.py (lines 2776-3350, 2017-2120, 3038-3145), /app/frontend/src/pages/LeadsPage.jsx (lines 2440-2800).
