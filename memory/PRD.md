...
|----------|--------|--------------|
| **Android APK** | ✅ Finished | https://expo.dev/artifacts/eas/cfS6VJpxR7evkaEn6AzBFG.apk |
...
## Changelog
- v5.1 (Feb 22, 2026): **Lead Investigator Audit Trail** - Complete debugging tool for Meta WhatsApp leads: (1) **Bug Fix** - Fixed NameError in webhook by adding missing To, AccountSid, NumMedia Form parameters. (2) **Audit Trail Storage** - Webhook now stores comprehensive audit data with 6 sections: raw_twilio_params (complete Twilio data), parsed_standard_fields, parsed_ctwa_fields (CtwaClid, ReferralHeadline, etc.), extraction_log (AD ID extraction steps), city_lookup_log (4-strategy city matching), final_assignment. (3) **Lead Investigator UI** - New expandable "Webhook Audit Trail" section in Lead Investigator modal shows all debugging data with color-coded status. (4) **Conclusion Box** - Green/amber/red status explaining if AD ID was captured correctly. 100% test coverage (iteration_65). Files: /app/backend/server.py (lines 2776-3350, 2017-2120), /app/frontend/src/pages/LeadsPage.jsx (lines 2440-2800).
- v5.0 (Feb 20, 2026): **Mechanic App Test Data & Map Navigation** - Two major features for mechanic mobile app testing: (1) **Test Data Seeding** - POST /api/mechanic/seed-test-data creates 6 sample inspections in Bangalore, Hyderabad, Chennai with statuses NEW, ACCEPTED, COMPLETED. DELETE /api/mechanic/clear-test-data removes all test data. (2) **Map Navigation Fix** - Updated API response to include latitude/longitude fields. Fixed frontend field mapping (vehicleNumber, makeModelVariant, customerName, customerPhone, customerAddress). Navigate button opens device maps with inspection location. (3) **Dev Mechanic Auth** - Fixed get_current_user to handle dev-mechanic-001 without database entry. Test credentials: Phone 9611188788, OTP 123456. 100% backend test coverage (iteration_64). Files: /app/backend/server.py (lines 282-370, 10807-10960, 11403-11520), /app/mechanic-app-native/app/home.tsx.
- v4.9 (Feb 20, 2026): **OBD Scanning UI State Fix** - Fixed critical issue where OBD scan completion was not reflected in the inspection-categories screen. Changes: (1) **InspectionContext Integration** - scanner.tsx now calls `setOBDScanResult()` upon scan completion, storing dtcCount, liveDataCount, sessionId, and timestamp. (2) **inspection-categories.tsx Update** - Reads `obdScanResult` from global context instead of using local state, ensuring UI updates correctly when user navigates back from scanner. (3) **New APK Build** - https://expo.dev/artifacts/eas/sx2Xrtr6Lavf88TjbswUZK.apk. Files modified: scanner.tsx (lines 48, 727-738), inspection-categories.tsx (lines 37-43).
- v4.8 (Feb 19, 2026): **Mechanic Native App - Full OBD Scanner UI Implementation** - Complete re-implementation of OBD-II Scanner UI migrated from original OBD repository: (1) **New Scanner Tab** - Added "OBD Scanner" tab in mechanic-app-native with full-featured BLE device scanning, ELM327 initialization, and DTC/Live Data scanning. (2) **Full UI Features** - Manufacturer selector (225+ manufacturers with year selection), Vehicle Information card, Connect & Scan button, device discovery list, scanning progress with module-by-module status. (3) **Results Display** - Summary cards (DTCs Found, Live Data, Duration), VIN display, DTC items with severity badges and category colors, Live Data grid with sensor values. (4) **Built-in Modals** - History modal (past scan sessions with DTCs and live data), Logs modal (debug logs with filtering and export), Raw Data modal (raw OBD command/response capture), DTC Detail modal (causes, symptoms, solutions). (5) **Tab Layout Update** - Updated tab layout to include: Inspections, OBD Scanner, History, Profile tabs. (6) **Scan Detail Page** - New /app/scan-detail/[id].tsx for viewing past scan sessions with full DTC breakdown and raw ECU responses. (7) **Services Integration** - Scanner UI connects to existing services: elm327.ts, dtc-parser-v2.ts, live-data-parser.ts, database.ts (scan sessions), logger.ts. TypeScript clean build (0 errors). Files: /app/mechanic-app-native/app/(tabs)/scanner.tsx (new), /app/mechanic-app-native/app/(tabs)/_layout.tsx (updated), /app/mechanic-app-native/app/scan-detail/[id].tsx (new).
- v4.7 (Feb 19, 2026): **Mechanic Push Notifications via FCM** - Complete push notification system for mechanic app: (1) **Push Token Management** - POST /api/mechanic/push-token registers FCM device token, DELETE removes on logout. Tokens stored in mechanic_push_tokens collection. (2) **Notification API** - GET /api/mechanic/notifications returns history, PATCH /{id}/read marks as read. (3) **Auto Triggers** - New inspection in city notifies all city mechanics via notify_mechanics_in_city(). Mechanic assignment triggers send_mechanic_notification(). (4) **Templates** - new_inspection_available 🚗, inspection_assigned ✅, inspection_reminder ⏰, inspection_cancelled ❌, payment_received 💰. (5) **Frontend** - Firebase SDK added, AuthContext manages token lifecycle, foreground handler shows toasts. FCM runs in MOCK MODE without production credentials. 100% backend test coverage (iteration_63).
- v4.6 (Feb 19, 2026): **Mechanic Mobile App Integration** - Complete integration of the mechanic app from GitHub repository for field inspections: (1) **OTP Authentication** - POST /api/auth/request-otp sends OTP to registered mechanic phone. POST /api/auth/verify-otp returns JWT token + profile. Dev mode OTP: 123456. (2) **Mechanic Inspection APIs** - GET /api/mechanic/inspections (list with filters), GET /api/mechanic/inspections/{id} (detail), POST accept/reject/progress/complete endpoints. (3) **Client-Specific Questionnaire** - InspectionChecklistPage now dynamically fetches questions from GET /api/inspections/{id}/questionnaire based on partner's inspection_template. Categories loaded from backend. (4) **File Uploads** - POST /api/uploads for photos/videos, GET /api/files/{filename} to serve. (5) **App Running** - Production build served on port 3001 via supervisor. Full data flow: Login → Dashboard (inspections) → Accept → Questionnaire → Complete. 100% backend test coverage (iteration_62).
