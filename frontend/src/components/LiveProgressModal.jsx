      ...
      <DialogContent className="sm:max-w-[1000px] max-h-[95vh] overflow-hidden flex flex-col p-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto">
          <div className="p-4 pb-6">
              ...
              {/* AI Category Ratings - Detailed ratings from AI analysis */}
              {inspection?.ai_insights?.category_ratings && Object.keys(inspection.ai_insights.category_ratings).length > 0 && (
                <Section title="AI Category Ratings (Detailed)" icon={BarChart3} defaultOpen={false}>
                  <p className="text-xs text-gray-500 mb-3">
                    Detailed category-wise ratings generated by AI analysis. Scale: 1-10 (1-3 = FAIL, 4-6 = ATTENTION, 7-10 = PASS)
                  </p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {Object.entries(inspection.ai_insights.category_ratings).map(([categoryName, ratingData]) => {
                      const rating = typeof ratingData === 'object' ? ratingData.rating : ratingData;
                      const status = typeof ratingData === 'object' ? ratingData.status : (rating >= 7 ? 'PASS' : rating >= 4 ? 'ATTENTION' : 'FAIL');
                      const summary = typeof ratingData === 'object' ? ratingData.summary : null;
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-medium text-gray-700 truncate flex-1">{categoryName}</span>
                            <span className={`ml-2 text-xs px-2 py-0.5 rounded font-semibold ${
                              status === 'PASS' ? 'bg-green-500 text-white' :
                              status === 'ATTENTION' ? 'bg-yellow-500 text-white' :
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-bold text-gray-700 w-8">{rating}/10</span>
                          </div>
                          {summary && (
